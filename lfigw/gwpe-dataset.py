...
#         else:
    ...
    def load_dataset(self, batch_size=512, detectors=None,
        ...
        # pytorch wrappers
        self.wfd_train = wfg.WaveformDatasetTorch(self.wfd, train=True)
        self.wfd_test = wfg.WaveformDatasetTorch(self.wfd, train=False)

        # DataLoader objects
        self.train_loader = DataLoader(
            self.wfd_train, batch_size=batch_size, shuffle=True, pin_memory=True,
            num_workers=16,
            worker_init_fn=lambda _: np.random.seed(
                int(torch.initial_seed()) % (2**32-1)))
        self.test_loader = DataLoader(
            self.wfd_test, batch_size=batch_size, shuffle=False, pin_memory=True,
            num_workers=16,
            worker_init_fn=lambda _: np.random.seed(
                int(torch.initial_seed()) % (2**32-1)))
...
def main():
    ...
    train_loader = DataLoader(
        pm.wfd_train, batch_size=500, shuffle=False, pin_memory=True,
        num_workers=16,
        worker_init_fn=lambda _: np.random.seed(
            int(torch.initial_seed()) % (2**32-1)))
    test_loader = DataLoader(
        pm.wfd_test, batch_size=500, shuffle=False, pin_memory=True,
        num_workers=16,
        worker_init_fn=lambda _: np.random.seed(
            int(torch.initial_seed()) % (2**32-1)))
        
    address = 'PCL_GWML_demo/data/{}/'.format(args.model_dir.split('/')[-2])
    try:
        os.makedirs(address)
    except:
        print(address, 'is exist!')
    
    y = np.concatenate([data[0].numpy() for data in train_loader])
    print(y.shape)
    np.save(address+'dataset_train_y', y)
    x = np.concatenate([data[1].numpy() for data in train_loader])
    print(x.shape)
    np.save(address+'dataset_train_x', x)
    w = np.concatenate([data[2].numpy()[...,np.newaxis] for data in train_loader])
    print(w.shape)
    np.save(address+'dataset_train_w', w)
    snr = np.concatenate([data[3].numpy()[...,np.newaxis] for data in train_loader])
    print(snr.shape)
    np.save(address+'dataset_train_snr', snr)


    y = np.concatenate([data[0].numpy() for data in test_loader])
    print(y.shape)
    np.save(address+'dataset_test_y', y)
    x = np.concatenate([data[1].numpy() for data in test_loader])
    print(x.shape)
    np.save(address+'dataset_test_x', x)
    w = np.concatenate([data[2].numpy()[...,np.newaxis] for data in test_loader])
    print(w.shape)
    np.save(address+'dataset_test_w', w)
    snr = np.concatenate([data[3].numpy()[...,np.newaxis] for data in test_loader])
    print(snr.shape)
    np.save(address+'dataset_test_snr', snr)
